...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] = {
      List[(Block, Move)]((left, Left), (right, Right), (up, Up), (down, Down))
    }
    ...
    def legalNeighbors: List[(Block, Move)] = {
      val l = List[(Block, Move)]((left, Left), (right, Right), (up, Up), (down, Down))
      l.filter((z: (Block, Move)) => (z._1.isLegal))
    }

    /**
     * Returns `true` if the block is standing.
     */
    def isStanding: Boolean = this match {
      case Block(b1: Pos, b2: Pos) => b1.x == b2.x && b1.y == b2.y
      case _ => false
    }
     ...
     */
    def isLegal: Boolean = this match {
      case Block(b1: Pos, b2: Pos) => terrain(b1) && terrain(b2)
      case _ => false
    }
